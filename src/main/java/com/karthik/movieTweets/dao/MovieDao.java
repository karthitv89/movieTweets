package com.karthik.movieTweets.dao;
...
public class MovieDao extends BaseDao {
	...
	public static String TWEETS_COLLECTION = "tweets";
	@Autowired
	...
	public MovieDao(MongoClient mongoClient, @Value("${spring.mongodb.database}") String databaseName) {
		...
		moviesCollection = db.getCollection(TWEETS_COLLECTION);
	}

	public Document addUser(String hashTag, String msg) {
		try {
			Document doc = new Document("hashTag", hashTag).append("msg", msg);
			moviesCollection.withWriteConcern(WriteConcern.MAJORITY).insertOne(doc);
			return doc;
		} catch (MongoException ex) {
			System.err.println("An error ocurred while trying to insert a User.");
			if (ErrorCategory.fromErrorCode(ex.getCode()) == ErrorCategory.DUPLICATE_KEY) {
				throw new MongoException("The User is already in the database.");
			}
			return null;
	...
	private boolean validIdValue(String movieId) {
		...
		try {
			...
			// Error handling code...
			System.out.println("VALIDIDVALUE======================= ex" + ex.getMessage());
			return false;
		}
	}
	...
	public Document getMovie(String movieId) {
		...
		if (!validIdValue(movieId)) {
			System.out.println("Not validIdValue&&&&& \n");
			return null;
		}
	...
	public List<Document> getMoviesByCountry(String... country) {
		...
		Bson queryFilter = new Document("title", new Document("$in", Arrays.asList(country)));
		Bson projection = new Document("title", 1).append("_id", 0);
		List<Document> movies = new ArrayList<>();
		// moviesCollection.find(queryFilter).projection(projection).into(movies);
		moviesCollection.aggregate(Arrays.asList(match(in("countries", Arrays.asList(country))),
				project(fields(include("title")/* , excludeId() */)))).into(movies);
